#!/usr/bin/env python3
"""Generate V39__agreement_firebase_v1_1_0.sql migration."""
import os
...
OUT = os.path.join(
    ...
    "db",
    "migration",
    "V39__agreement_firebase_v1_1_0.sql",
)

# ---------------------------------------------------------------------------
# Shared changelog block (appended to the end of each doc)
# ---------------------------------------------------------------------------
CHANGELOG_JA = """\
---
...
# ---------------------------------------------------------------------------
# USER_TERMS — section 10 updated in all languages
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PRIVACY_POLICY — Firebase sections updated
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THIRD_PARTY_SDK — Firebase entries added, removed from "not integrated"
# ---------------------------------------------------------------------------
...
AGREEMENTS = [
    ("USER_TERMS", "ja", "利用規約", USER_TERMS_JA, True),
    ("USER_TERMS", "en", "User Terms", USER_TERMS_EN, True),
    ("USER_TERMS", "zh", "用户协议", USER_TERMS_ZH, True),
    ("PRIVACY_POLICY", "ja", "プライバシーポリシー", PRIVACY_JA, True),
    ("PRIVACY_POLICY", "en", "Privacy Policy", PRIVACY_EN, True),
    ("PRIVACY_POLICY", "zh", "隐私政策", PRIVACY_ZH, True),
    ("THIRD_PARTY_SDK", "ja", "サードパーティ SDK 一覧", SDK_JA, False),
    ("THIRD_PARTY_SDK", "en", "Third-party SDK List", SDK_EN, False),
    ("THIRD_PARTY_SDK", "zh", "第三方 SDK 一览", SDK_ZH, False),
]
...
def format_doc(body: str, changelog: str) -> str:
    """让元信息每条一行，并把更新日志追加到文档末尾。

    Markdown 中相邻的非空行会被合并成同一段（软换行变空格），导致
    `版本 / 生效日期 / 最后更新` 挤在一行。这里给元信息行补两个行尾空格，
    形成硬换行，让它们各占一行。更新日志统一放在正文最后。
    """
    lines = body.split("\n")
    out = []
    prev_meta = False
    for line in lines:
        is_meta = line.startswith("**") and ("：" in line or ": " in line)
        if is_meta:
            # 元信息每条独立成段（前后空行），比行尾两空格的硬换行更稳健：
            # 行尾空格常被编辑器 / 格式化工具 / Git 清掉，导致换行失效。
            ...
            prev_meta = True
            continue
        if prev_meta and line.strip() == "":
            # 元信息块后已补过空行，跳过原文这一空行避免重复
            prev_meta = False
            continue
        prev_meta = False
        out.append(line)
...
def sql_insert(schema: str, code: str, lang: str, title: str, content: str, required: bool) -> str:
    req = "TRUE" if required else "FALSE"
    # Escape single quotes in content for SQL dollar-quoting we use $body$
    return f"""
-- ---- {schema}: {code} / {lang} / 1.1.0 -----------------------------------
WITH new_row AS (
  SELECT
...
""",
    ...
    for schema in ("public", "crawl"):
        for code, lang, title, content, required in AGREEMENTS:
            doc = format_doc(content, CHANGELOGS[lang])
            parts.append(sql_insert(schema, code, lang, title, doc, required))
