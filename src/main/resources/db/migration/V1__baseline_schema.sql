...
CREATE TABLE IF NOT EXISTS movie (
    ...
    want_to_see_count INTEGER DEFAULT 0,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS movie_tag_tags (
    ...
    deleted INTEGER DEFAULT 0,
    FOREIGN KEY (movie_id) REFERENCES movie(id) ON DELETE CASCADE,
    FOREIGN KEY (movie_tag_id) REFERENCES movie_tag(id) ON DELETE CASCADE
);

-- 电影场次表：依赖 cinema / theater_hall，已挪到“影院相关表”之后创建（见下方 theater_hall 之后）。
...
-- 地区表（日本行政区字典；V12 会从 public 同步到 crawl）
...
-- 影院规格字典（IMAX / 4DX 等；与 cinema_spec_spec 关联表配合使用）
...
CREATE TABLE IF NOT EXISTS cinema_spec_spec (
    ...
    FOREIGN KEY (cinema_id) REFERENCES cinema(id) ON DELETE CASCADE,
    FOREIGN KEY (spec_id) REFERENCES cinema_spec(id) ON DELETE CASCADE
);
...
-- 电影场次表（依赖 movie / cinema / theater_hall，三者均已在前面创建）
...
-- 电影订单表
-- 2026-05-28: 订单主键改为 BIGINT identity，避免订单量增长后触达 INTEGER 上限。
...
-- 映倫分级表（G / PG-12 / R-15 / R-18；爬虫写 movie.level_id）
...
-- 2026-05-28: 兼容已存在但缺少 release_date 的 movie 表，避免创建索引时失败。
